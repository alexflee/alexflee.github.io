...
class Navee_mcp {
	...
	function Navee_mcp(){
		...
		if (is_dir($this->EE->config->item('theme_folder_path').'third_party/navee')){
			$this->theme_folder_directory = "third_party/";
		}
	...
	function index(){
		...
		$this->EE->cp->add_to_head('<link rel="stylesheet" type="text/css" href="'.$this->EE->config->item('theme_folder_url').$this->theme_folder_directory.'navee/css/navee.css" />');
		...
		// Check to see if there is anything in the database
			...
			$vars["theme_folder_url"]	= $this->EE->config->item('theme_folder_url').$this->theme_folder_directory;	
	...
	function config(){
		...
		$this->EE->cp->set_variable('cp_page_title', $this->EE->lang->line('cp_conf'));
	...
	function config_handler(){
		...
		// Determine if there is an existing EE subdirectory
		$this->EE->db->select("v");
		$this->EE->db->where("k", "install_directory");
		$this->EE->db->where("site_id", $this->site_id);
		$q = $this->EE->db->get("navee_config");
			...
			if ($q->num_rows() > 0){
				$row = $q->row();
				$oldInstallDirectory = $row->v;
			}
		$q->free_result();
		
		// If there is an existing EE subdirectory, and it has changed - update links
		if (strlen($oldInstallDirectory) > 0){
			$this->EE->db->select("navee_id, link");
			$this->EE->db->like("link", $oldInstallDirectory, "after");
			$this->EE->db->where("site_id", $this->site_id);
			...
			if ($q->num_rows() > 0){
				foreach ($q->result() as $row){
					...
					$regex = str_replace("/","\/",$oldInstallDirectory);
					$newLink = preg_replace("/^".$regex."\//i", $this->EE->input->post("install_directory")."/", $row->link,1);
					...
					$this->EE->db->where("site_id", $this->site_id);
					$this->EE->db->where('navee_id', $row->navee_id);
				...
				$returnMe .= "<form class='naveeCPForm' onsubmit='return false;' name='naveeCPForm_".$r->navee_id."'>
									...
									<ol class='required'>
										<li>
											...
											<input type='input' name='navee_text' value='".htmlspecialchars($r->text, ENT_QUOTES)."' />
										</li>
	...
	function _styleNavCP($nav){
		...
		if (sizeof($nav)>0){
			...
			foreach ($nav as $k=>$v){
				...
				$returnMe .= "<a class='navee_trash icn' title='".$this->EE->lang->line('cp_delete')."' href='javascript:;'><img src='".$this->EE->config->item('theme_folder_url').$this->theme_folder_directory."navee/img/trash.png' /></a><a class='navee_edit icn' title='".$this->EE->lang->line('cp_edit')."' href='javascript:;'><img src='".$this->EE->config->item('theme_folder_url').$this->theme_folder_directory."navee/img/edit.png' /></a><a class='navee_code icn' title='".$this->EE->lang->line('cp_code')."' href='javascript:;'><img src='".$this->EE->config->item('theme_folder_url').$this->theme_folder_directory."navee/img/process.png' /></a>";
				// Cut/Paste EE tags
	...
	function _stylePagesSelect($pages){
		if (strlen($this->ee_install_directory) > 0){
			$this->ee_install_directory = "/".$this->ee_install_directory;
		}
		sort($pages);
		...
		if (sizeof($pages)>0){
			foreach ($pages as $k=>$v){
				$returnMe .= "<option value='".$this->ee_install_directory.$v."'>".$this->ee_install_directory.$v."</option>";
			}
		}
		$returnMe .= "</select>";
		return $returnMe;
